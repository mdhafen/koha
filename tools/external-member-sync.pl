...
my $clear_msg_prefs_sth = $dbh->prepare(q{DELETE bmp FROM borrower_message_preferences AS bmp LEFT JOIN message_attributes AS ma USING (message_attribute_id) WHERE message_name IN ('Advance_Notice','Hold_Filled') AND  borrowernumber = ?});
