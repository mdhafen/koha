...
my $no_msg_branches = {'all'=>{},'holds'=>{}};
{
    my @groups = Koha::Library::Groups->search( { title => 'NoMessaging' } )->as_list();
    @groups = map { $_->all_libraries } @groups;
    foreach my $b ( @groups ) {
        $no_msg_branches->{'all'}{$b->branchcode} = 1;
    }

    @groups = Koha::Library::Groups->search( { title => 'NoHoldMessaging' } )->as_list();
    @groups = map { $_->all_libraries } @groups;
    foreach my $b ( @groups ) {
        $no_msg_branches->{'holds'}{$b->branchcode} = 1;
    }
}
# Clear message preferences for Advance_Notice
my $clear_msg_prefs_sth = $dbh->prepare(q{DELETE bmp FROM borrower_message_preferences AS bmp LEFT JOIN message_attributes AS ma USING (message_attribute_id) WHERE message_name <> 'Item_Checkout' AND  borrowernumber = ?});
my $clear_hld_msg_prefs_sth = $dbh->prepare(q{DELETE bmp FROM borrower_message_preferences AS bmp LEFT JOIN message_attributes AS ma USING (message_attribute_id) WHERE message_name IN ('Hold_Filled','Hold_Reminder') AND  borrowernumber = ?});

if ( ( $op eq 'Sync' || $op eq 'cud-sync' ) and $category ) {
#warn "Getting lists...";
        ...
        if ( defined $mapping{categorycode}->{is} ) {
            ...
            if(defined $ldap->{categorycode_mapping}) {
                ...
                foreach my $cat (@{$ldap->{categorycode_mapping}->{categorycode}}) {
                    if ( $cat->{content} eq $category ) {
                        push @cat_fil, "($field=". $cat->{value} .")";
                    }
                }
                if ( @cat_fil && scalar @cat_fil > 1 ) {
                    push @filters, "(|". join('',@cat_fil) .")"
                }
                elsif ( @cat_fil ) {
                    push @filters, $cat_fil[0];
                }
            }
            else {
...
#warn "checking for deletes...";
    if ( $dirhash && %$dirhash ) {  # to make sure the directory isn't empty.
        foreach my $cardnumber (sort keys %$dbhash) {
            ...
            $bordata->{'preferred_name'} ||= $dbhash->{$cardnumber}{'preferred_name'};
            $bordata->{'cardnumber'} ||= $cardnumber;
            $bordata->{'sort1'} ||= $dbhash->{$cardnumber}{'sort1'};
            $bordata->{'sort2'} ||= $dbhash->{$cardnumber}{'sort2'};

            my %this_report = (
                name => $bordata->{'surname'} .', '. $bordata->{'preferred_name'} . ( $bordata->{'preferred_name'} ne $bordata->{'firstname'} ? ' ('.$bordata->{'firstname'}.')' : '' ),
            ...
            if ( $bordata && $bordata->{'branchcode'} && ( $bordata->{'branchcode'} != $branch ) ) {
                ...
                if ( $confirmed ) {
                    if ( $historical_branch && ! $branches_map{$bordata->{'branchcode'}} ) {
                        ...
                        if (C4::Context->preference('EnhancedMessagingPreferences')) {
                            if ( ( $no_msg_branches->{'all'}{$dbhash->{$cardnumber}{'branchcode'}} || $no_msg_branches->{'holds'}{$dbhash->{$cardnumber}{'branchcode'}} ) && ! $no_msg_branches->{'all'}{$bordata->{'branchcode'}} ) {
                                ...
                                if ( $no_msg_branches->{'holds'}{$bordata->{'branchcode'}} ) {
                                    $clear_hld_msg_prefs_sth->execute($dbhash->{$cardnumber}{'borrowernumber'});
                                }
                            }
                            elsif ( $no_msg_branches->{'holds'}{$bordata->{'branchcode'}} ) {
                                $clear_hld_msg_prefs_sth->execute($dbhash->{$cardnumber}{'borrowernumber'});
                            }
                            elsif ( $no_msg_branches->{'all'}{$bordata->{'branchcode'}} ) {
                                $clear_msg_prefs_sth->execute($dbhash->{$cardnumber}{'borrowernumber'});
                            }
                        }
                    }
...
';
    ...
    foreach my $cardnumber ( sort keys %deleted ) {
        ...
        $numdeleted++;
        push @report, {
            'name' => $fields->{'surname'} .', '. $fields->{'preferred_name'}  . ( $fields->{'preferred_name'} ne $fields->{'firstname'} ? ' ('.$fields->{'firstname'}.')' : '' ),
...
#warn "Adding...";
    foreach my $cardnumber ( sort keys %added ) {
        ...
        if ( @fields ) {
            ...
            if ( $#fields > 1 ) {
                $added{$cardnumber}{'preferred_name'} ||= $fields[0]{'preferred_name'};
                # To many rows returned
                push @report, {
                    'name' => $added{$cardnumber}{'surname'} .', '. $added{$cardnumber}{'preferred_name'} . ( $added{$cardnumber}{'preferred_name'} ne $added{$cardnumber}{'firstname'} ? ' ('.$added{$cardnumber}{'firstname'}.')' : '' ),
                    'cardnumber' => $cardnumber,
                    'duplicate' => 1,
                };
            }
            ...
            if ( $confirmed ) {
                ...
                if ( $@ ) {
                    ...
                    $attribs->{'preferred_name'} ||= $attribs->{'firstname'};

                    push @report, {
                        'name' => $attribs->{'surname'} .', '. $attribs->{'preferred_name'} . ( $attribs->{'preferred_name'} ne $attribs->{'firstname'} ? ' ('.$attribs->{'firstname'}.')' : '' ),
                        'cardnumber' => $cardnumber,
                        'error' => 1,
                        'create_error' => 1,
                    };
                    ...
                    if (C4::Context->preference('EnhancedMessagingPreferences')) {
                        unless ( $no_msg_branches->{'all'}{$attribs->{'branchcode'}} ) {
                            C4::Members::Messaging::SetMessagingPreferencesFromDefaults({ borrowernumber => $patron->borrowernumber, categorycode => $category });
                        }
                        if ( $no_msg_branches->{'holds'}{$attribs->{'branchcode'}} ) {
                            $clear_hld_msg_prefs_sth->execute($patron->borrowernumber);
                        }
                    }
                }
            }
...
Sync_Existing:
    foreach my $cardnumber ( sort keys %existing ) {
        ...
        if ( $#fields > 1 ) {
            $existing{$cardnumber}{'preferred_name'} ||= $fields[0]{'preferred_name'};
            # To many rows returned
            push @report, {
                'name' => $existing{$cardnumber}{'surname'} .', '. $existing{$cardnumber}{'preferred_name'} . ( $existing{$cardnumber}{'preferred_name'} ne $existing{$cardnumber}{'firstname'} ? ' ('.$existing{$cardnumber}{'firstname'}.')' : '' ),
                'cardnumber' => $cardnumber,
                'duplicate' => 1,
            };
        }
            ...
            foreach ( sort @mapped_fields ) {
                ...
                if ( exists $attribs->{ $_ } && defined $attribs->{ $_ } ) {
                    if ( ( $_ eq 'userid' || $_ eq 'cardnumber' ) && ( ! defined $values->{$_} || $attribs->{$_} ne $values->{$_} ) ) {
                        my $dups = $dbh->selectall_arrayref( "SELECT * FROM borrowers WHERE $_ = ". $dbh->quote($attribs->{$_}) );
                        if ( scalar @$dups ) {
                            push @report, {
                                'name' => $values->{'surname'} .', '. $values->{'preferred_name'} . ( $values->{'preferred_name'} ne $values->{'firstname'} ? ' ('.$values->{'firstname'}.')' : '' ),
                                'cardnumber' => $cardnumber,
                                'duplicate' => 1,
                                'message' => "$_ = ". $attribs->{$_},
                            };
            ...
            if ( ! $attribs->{'preferred_name'} && $values->{'preferred_name'} eq $values->{'firstname'} && $attribs->{'firstname'} ne $values->{'firstname'} ) {
                $attribs->{'preferred_name'} = $attribs->{'firstname'};
                push @changes, { field => 'preferred_name', old => $values->{'preferred_name'}, new => $attribs->{'preferred_name'} };
            }
            ...
            if ( $diff ) {
                if ( $confirmed ) {
                    ...
                    if ( $attribs->{'branchcode'} && C4::Context->preference('EnhancedMessagingPreferences') ) {
                        if ( ( $no_msg_branches->{'all'}{$values->{'branchcode'}} || $no_msg_branches->{'holds'}{$values->{'branchcode'}} ) && ! $no_msg_branches->{'all'}{$attribs->{'branchcode'}} ) {
                            C4::Members::Messaging::SetMessagingPreferencesFromDefaults({ borrowernumber => $values->{'borrowernumber'}, categorycode => $category });
                            if ( $no_msg_branches->{'holds'}{$attribs->{'branchcode'}} ) {
                                $clear_hld_msg_prefs_sth->execute($values->{'borrowernumber'});
                            }
                        }
                        elsif ( $no_msg_branches->{'holds'}{$attribs->{'branchcode'}} ) {
                            $clear_hld_msg_prefs_sth->execute($values->{'borrowernumber'});
                        }
                        elsif ( $no_msg_branches->{'all'}{$attribs->{'branchcode'}} ) {
                            $clear_msg_prefs_sth->execute($values->{'borrowernumber'});
                        }
                    }
                }

                $attribs->{'preferred_name'} ||= $values->{'preferred_name'};
                push @report, {
                    'name' => $attribs->{'surname'} .', '. $attribs->{'preferred_name'} . ( $attribs->{'preferred_name'} ne $attribs->{'firstname'} ? ' ('.$attribs->{'firstname'}.')' : '' ),
                    'cardnumber' => $cardnumber,
                    'changed' => \@changes,
