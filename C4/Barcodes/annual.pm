...
sub db_max {
	my $self = shift;
	my $query = "SELECT substring_index(barcode,'-',-1) AS chunk,barcode FROM items WHERE barcode LIKE ? AND homebranch = '". C4::Context->userenv->{'branch'} ."' ORDER BY chunk DESC LIMIT 1";
