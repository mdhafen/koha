  ...
  exclude-result-prefixes="marc items">
    ...
    <xsl:template match="marc:record">
        ...
        <xsl:choose>
            <xsl:when test="marc:subfield[@code=9]">
                <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?idx=an&amp;q=<xsl:value-of select="marc:subfield[@code=9]"/></xsl:attribute>
            </xsl:when>
            <xsl:otherwise>
            <xsl:attribute name="href">/cgi-bin/koha/catalogue/search.pl?idx=au&amp;q=<xsl:value-of select="marc:subfield[@code='a']"/></xsl:attribute>
            </xsl:otherwise>
        </xsl:choose>
   ...
   <xsl:if test="$materialTypeCode!=''">
        ...
        <xsl:for-each select="marc:datafield[@tag=440]">
             <a href="/cgi-bin/koha/catalogue/search.pl?idx=se&amp;q={marc:subfield[@code='a']}">
        ...
        <span class="results_summary"><span class="label">Publisher: </span>
            <xsl:for-each select="marc:datafield[@tag=260]">
                <xsl:if test="marc:subfield[@code='b']">
                <a href="/cgi-bin/koha/catalogue/search.pl?idx=pb&amp;q={marc:subfield[@code='b']}">
                    <xsl:call-template name="subfieldSelect">
                        <xsl:with-param name="codes">b</xsl:with-param>
                    </xsl:call-template>
               </a>
