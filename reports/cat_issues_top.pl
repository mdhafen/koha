...
#
    ...
    # 	$colfilter[0] = @$filters[11] if ($column =~ /sort2/ ) ;
        $colfilter[0] = @$filters[7] if ($column =~ /issuedate/ ) ;
        $colfilter[0] = @$filters[8] if ($column =~ /issuedate/ ) ;
...
# Processing average loanperiods
    ...
    $strcalc .= " AND dayname(old_issues.issuedate) like '" . @$filters[7]."'" if (@$filters[7]);
    @$filters[8]=~ s/\*/%/g if (@$filters[8]);
    $strcalc .= " AND monthname(old_issues.issuedate) like '" . @$filters[8]."'" if (@$filters[8]);
    @$filters[9]=~ s/\*/%/g if (@$filters[9]);
    $strcalc .= " AND year(old_issues.issuedate) like '" . @$filters[9] ."'" if ( @$filters[9] );
