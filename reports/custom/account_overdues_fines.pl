...
if ( $do_it ) {
    ...
    $query = "( SELECT ". ($no_sort2 ? '' : 'borrowers.sort2,') . ($concise ? '' : 'borrowers.cardnumber,') ." CONCAT_WS(', ',borrowers.surname,borrowers.firstname) AS patron, description, amountoutstanding, borrowers.cardnumber, borrowers.borrowernumber FROM accountlines CROSS JOIN borrowers USING (borrowernumber) LEFT JOIN items USING (itemnumber) WHERE amountoutstanding <> 0";

    $query .= ' AND '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @q_filters ) ." " if (@q_filters);

    $query .= " ) UNION ( SELECT ". ($no_sort2 ? '' : 'borrowers.sort2,') . ($concise ? '' : 'borrowers.cardnumber,') ." CONCAT_WS(', ',borrowers.surname,borrowers.firstname) AS patron, ". ($concise ? "CONCAT_WS( ' &nbsp; ', barcode, CONCAT_WS( ' ', biblio.title, biblio.subtitle ) )" : "CONCAT_WS( '<br>', CONCAT_WS( ' ', biblio.title, biblio.subtitle ), CONCAT( 'Due: ', date_due ), CONCAT( 'Barcode: ', barcode, ' (', branchname,') &nbsp; Call Number: ', COALESCE(itemcallnumber,'') ), CONCAT( '<b>Replacement Price: ', replacementprice, '</b>' ) )") ." AS description, ". ($concise? 'replacementprice' : 'NULL') .", borrowers.cardnumber, borrowers.borrowernumber FROM issues CROSS JOIN borrowers USING (borrowernumber) CROSS JOIN items USING (itemnumber) CROSS JOIN biblio USING (biblionumber) CROSS JOIN branches on homebranch = branches.branchcode WHERE ". ($input->param('checked_out') ? "0 = 0" : "date_due < CURRENT_DATE()" );
