...
$template->param(
    report_file => 'circ_reserves',
    report_title => 'Titles on hold',
    report_desc => '',
    do_it => $do_it,
...
my @parameters = (
    {
        ...
        select_loop => [
            ...
            { value => 'borrower', label => 'Patron' },
            { value => 'date',     label => 'Date Placed' },
        ],
    },
);
...
my $query = "";
my @query_params;
#  watch branch param, which changes callnumbers column
my @columns = ( "CONCAT( borrowers.surname, ', ', borrowers.firstname ) AS borrower", "branchname", "CONCAT_WS(' ', biblio.title, biblio.subtitle ) AS title", "( SELECT GROUP_CONCAT(DISTINCT itemcallnumber) FROM items where biblionumber=reserves.biblionumber group by biblionumber) as callnumbers", "reservedate", "priority", "COALESCE(avail_items.num_avail,0) AS num_avail", "reserves.biblionumber" );
my $where = "( found <> 'F' OR found IS NULL )";
my $order = "reservedate DESC, title, priority";
...
if ( $do_it ) {
    ...
    for ( scalar $input->param("order") ) {
        if    ( $_ eq 'title' )    { $order = 'title,priority' }
        elsif ( $_ eq 'borrower' ) { $order = 'borrower,title' }
        elsif ( $_ eq 'date' )     { $order = 'reservedate DESC, title, priority' }
    }
	...
	$columns[3] = "( SELECT GROUP_CONCAT(DISTINCT itemcallnumber) FROM items where homebranch = ". $dbh->quote($branch) ." AND biblionumber=reserves.biblionumber group by biblionumber) as callnumbers";
        push @filters, { crit => 'reserves.branchcode', op => '=', filter => $dbh->quote($branch), label => 'Library', value => $branchname };
    }

    $query = "SELECT ". (join ',',@columns) ." FROM reserves
 CROSS JOIN borrowers USING (borrowernumber)
 LEFT JOIN biblio USING (biblionumber)
 CROSS JOIN branches ON borrowers.branchcode = branches.branchcode
 LEFT JOIN (
  SELECT COUNT(*) AS num_avail, biblionumber
  FROM items
  LEFT JOIN issues USING (itemnumber)
  WHERE issues.itemnumber IS NULL
   ". ( $branch ? " AND homebranch = ". $dbh->quote($branch) : "" ) ."
   AND COALESCE(items.withdrawn,0) = 0
   AND COALESCE(items.restricted,0) = 0
   AND COALESCE(items.itemlost,0) = 0 GROUP BY biblionumber) AS avail_items USING (biblionumber)
 WHERE $where ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[2]->{link} = "/cgi-bin/koha/reserve/request.pl?biblionumber=$values[7]";
        $results[4]->{'is_date'} = 1;
