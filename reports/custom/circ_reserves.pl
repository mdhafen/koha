...
my $query = "";
my @query_params;
my @columns = ( "CONCAT( borrowers.surname, ', ', borrowers.firstname ) AS borrower", "branchname", "CONCAT_WS(' ', biblio.title, biblio.subtitle ) AS title", "avail_items.callnumbers", "reservedate", "priority", "avail_items.num_avail", "reserves.biblionumber" );
...
if ( $do_it ) {
 ...
 LEFT JOIN (
  SELECT COUNT(*) AS num_avail, GROUP_CONCAT(DISTINCT itemcallnumber) AS callnumbers, biblionumber
  FROM items
