...
$template->param(
    report_file => 'circ_monthly_total',
    report_title => 'Monthly circulation totals',
    report_desc => '',
    do_it => $do_it,
...
my $where = "";
my $order = "year DESC, month DESC";
my $group = "YEAR(issuedate), MONTH(issuedate)";
...
if ( $do_it ) {
    ...
    $query .= "SELECT issues.* FROM issues LEFT JOIN items USING (itemnumber) ";
    $query .= 'WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @q_filters ) ." " if (@q_filters);
    $query .= "UNION SELECT old_issues.* FROM old_issues LEFT JOIN items USING (itemnumber) ";
    $query .= 'WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @q_filters ) ." " if (@q_filters);
    $query .= ") AS all_issues ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        my $date = sprintf("%04d",$values[0]) ."-". sprintf("%02d",$values[1]) ."-01";
        $date = dt_from_string($date)->strftime(qq(%b %Y));
        my @results = (
            { value => $date },
            { value => $values[2] },
        );
