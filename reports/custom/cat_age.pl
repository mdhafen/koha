...
my $query = "";
my @query_params;
my @columns = ( "copyrightdate", "CONCAT_WS(' ', biblio.title, biblio.subtitle ) AS title", "itemcallnumber", "barcode", "COALESCE(num_issues,0)", "biblionumber" );
...
if ( $do_it ) {
    ...
    if ( $input->param("only_one") ) {
        ...
        $columns[4] = 'SUM(COALESCE(num_issues,0))';
        $group = "biblionumber";
