...
my $query = "";
my @query_params;
my @columns = ( "CONCAT_WS(', ',borrowers.surname,borrowers.firstname) AS patron", "cardnumber", "message_date", "br.branchname", "IF( message_type = 'B', 'Borrower', 'Librarian' )", "message", "message_id", "borrowernumber" );
my $order = "patron";
...
if ( $do_it ) {
    ...
    $query = "SELECT ". (join ',',@columns) ." FROM messages CROSS JOIN borrowers USING (borrowernumber) LEFT JOIN branches AS br ON messages.branchcode = br.branchcode ";
