...
$template->param(
    report_file => 'circ_stats_noissues',
    report_title => 'Copies with no check outs',
    report_desc => '',
    do_it => $do_it,
...
my @parameters = (
    {
        ...
        calendar => 1,
        to_id => 'cr_to',
        value => dt_from_string()->subtract(years=>1),
    },
    {
        input_id => 'cr_to',
        input_name => 'to',
...
my @columns = ( "itemcallnumber", "barcode", "copyrightdate", "title", "branchname", "items.biblionumber" );
my $where = "all_issues.date_due IS NULL";
my $order = "title";
my $group = "";
...
if ( $do_it ) {
    if ( my $val = $input->param('from') ) {
        ...
        push @u_params, $val;
        push @filters, { label => 'From', op => '>', value => $val };
    }
    ...
    if ( my $val = $input->param('to') ) {
        ...
        push @u_filters, { crit => 'issuedate', op => '<', filter => '?' };
        push @u_params, $val;
        push @filters, { label => 'To', op => '<', value => $val };
    }
 ...
 LEFT JOIN branches ON homebranch = branchcode
 LEFT JOIN (
  SELECT issuedate,date_due,itemnumber FROM issues";
    $query .= ' WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @u_filters ) ." " if (@u_filters);
    $query .= "
  UNION SELECT issuedate,date_due,itemnumber FROM old_issues";
    $query .= ' WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @u_filters ) ." " if (@u_filters);
    $query .= "
 ) AS all_issues USING (itemnumber) WHERE $where ";
    ...
    if ( @u_params ) {
        unshift @query_params, @u_params, @u_params;
    }
}
