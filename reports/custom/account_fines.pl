...
my @column_titles = ( "Homeroom Teacher", "Patron", "Cardnumber", "Title", "Type", "Description", "Amount Outstanding" );

my $query = "";
my @query_params;
my @columns = ( "borrowers.sort2", "CONCAT_WS(', ',borrowers.surname,borrowers.firstname) AS patron", "borrowers.cardnumber", "CONCAT_WS(' ',biblio.title,biblio.seriestitle) AS title", "IF(COALESCE(debit_type_code,'') <> '',account_debit_types.description,account_credit_types.description) AS type", "accountlines.description", "accountlines.amountoutstanding", "borrowers.borrowernumber", "borrowers.cardnumber" );
...
if ( $do_it ) {
    ...
    if ( ( my $list_id = $input->param('plist') ) || ( $input->param("per_page") && $input->param("per_page") eq 'patron_list_id' ) || ( $input->param("order") && $input->param("order") eq 'list' ) ) {
        ...
        $query = "SELECT ". (join ',',@columns) ." FROM accountlines LEFT JOIN account_debit_types ON (account_debit_types.code = debit_type_code) LEFT JOIN account_credit_types ON (account_credit_types.code = credit_type_code) CROSS JOIN borrowers USING (borrowernumber) LEFT JOIN patron_list_patrons USING (borrowernumber) LEFT JOIN patron_lists USING (patron_list_id) LEFT JOIN items USING (itemnumber) LEFT JOIN biblio USING (biblionumber) WHERE $where ";
    }
    else {
        $query = "SELECT ". (join ',',@columns) ." FROM accountlines LEFT JOIN account_debit_types ON (account_debit_types.code = debit_type_code) LEFT JOIN account_credit_types ON (account_credit_types.code = credit_type_code) CROSS JOIN borrowers USING (borrowernumber) LEFT JOIN items USING (itemnumber) LEFT JOIN biblio USING (biblionumber) WHERE $where ";
    }
