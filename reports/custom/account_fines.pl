...
if ( $do_it ) {
    ...
    if ( ( my $list_id = $input->param('plist') ) || ( $input->param("per_page") && $input->param("per_page") eq 'patron_list_id' ) || ( $input->param("order") && $input->param("order") eq 'list' ) ) {
        ...
        $query = "SELECT ". (join ',',@columns) ." FROM accountlines CROSS JOIN borrowers USING (borrowernumber) LEFT JOIN patron_list_patrons USING (borrowernumber) LEFT JOIN patron_lists USING (patron_list_id) LEFT JOIN items USING (itemnumber) LEFT JOIN biblio USING (biblionumber) WHERE $where ";
    }
    else {
