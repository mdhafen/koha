...
$template->param(
    report_file => 'circ_all_issues',
    report_title => 'All check outs',
    report_desc => '',
    do_it => $do_it,
...
my $query = "";
my @query_params;
my @outer_columns = ( "issuedate", 'renewals_count', "COALESCE(returndate,'') AS returndate", 'date_due', 'borrowers.sort2', "CONCAT_WS(', ',borrowers.surname,borrowers.firstname) AS patron", 'branches.branchname', "CONCAT_WS(' ',biblio.title,biblio.subtitle) AS title", 'all_issues.itemcallnumber', 'all_issues.barcode', 'all_issues.replacementprice', "COALESCE(all_issues.itemnotes,'') AS itemnotes", 'borrowernumber', 'biblionumber', "all_issues.itemnumber" );
my @columns = ( "issuedate", "renewals_count", "returndate", "date_due", "itemcallnumber", "barcode", "replacementprice", "CONCAT_WS('\n',itemnotes,itemnotes_nonpublic) AS itemnotes", "borrowernumber", "itemnumber" );
my $where = "";
my $order = "issuedate, patron";
my $group = "";
...
if ( $do_it ) {
    ...
    $query = "SELECT ". (join ',',@outer_columns) ." FROM ";
    $query .= "( SELECT ". (join ',',@columns) ." FROM issues AS i LEFT JOIN items USING (itemnumber) ";
    $query .= 'WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @q_filters ) ." " if (@q_filters);
    $query .= "UNION ALL SELECT ". (join ',',@columns) ." FROM old_issues AS i LEFT JOIN items USING (itemnumber) ";
    $query .= 'WHERE '. join( ' AND ', map { "$_->{crit} $_->{op} $_->{filter}" } @q_filters ) ." " if (@q_filters);
    $query .= ") AS all_issues LEFT JOIN borrowers USING (borrowernumber) LEFT JOIN items USING (itemnumber) LEFT JOIN biblio USING (biblionumber) LEFT JOIN branches ON borrowers.branchcode = branches.branchcode ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[2]->{'is_date'} = 1;
        $results[3]->{'is_date'} = 1;
        $results[10]->{value} = sprintf( "%.2f", $results[10]->{value} );
        $results[5]->{link} = "/cgi-bin/koha/members/moremember.pl?borrowernumber=$values[12]";
        $results[7]->{link} = "/cgi-bin/koha/catalogue/detail.pl?biblionumber=$values[13]";
        $results[9]->{link} = "/cgi-bin/koha/catalogue/moredetail.pl?biblionumber=$values[13]&itemnumber=$values[14]#item$values[14]";
