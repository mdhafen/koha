...
my $template_file = ( $do_it ? "results" : "cud-params" );
...
my @parameters = (
    ...
    },
    {
        input_id => 'cr_min_cn',
        input_name => 'minloc',
        label => 'Call number between',
        input_box => 1,
#        Commented out because cn_sort field needs processing with GetClassSort
#        crit => 'items.cn_sort',
        op => '>=',
    },
    {
...
if ( $do_it ) {
    ...
    if ( $input->param('minloc') ) {
        my $min_callnum = $input->param('minloc');
        push @filters, { crit => 'items.cn_sort', op => '>=', filter => $dbh->quote(GetClassSort( C4::Context->preference("DefaultClassificationSource"), undef, $min_callnum )), label => 'Min Call Number', value => $min_callnum };
    }

    if ( $input->param('maxloc') ) {
        my $max_callnum = $input->param('maxloc');
        push @filters, { crit => 'items.cn_sort', op => '<=', filter => $dbh->quote(GetClassSort( C4::Context->preference("DefaultClassificationSource"), undef, $max_callnum )), label => 'Max Call Number', value => $max_callnum };
    }
