...
my @column_titles = ( "Title", "Author", "Series Title", "Volume", "Copyright", "Library", "Call Number", "Barcode", "Item Type", "Copy Notes", "Availability" );

my $query = "";
my @query_params;
my @columns = ( "CONCAT_WS(' ',b1.title,b1.subtitle) AS title",
            "b1.author", "b1.seriestitle", "biblioitems.volume",
            "b1.copyrightdate", "items.homebranch", "items.itemcallnumber",
            "items.barcode", "items.itype", "CONCAT_WS('\n',items.itemnotes,items.itemnotes_nonpublic) AS item_notes",
...
if ( $do_it ) {
    ...
    $query = "SELECT ". (join ',',@columns) ." FROM items LEFT JOIN issues USING (itemnumber) CROSS JOIN biblio b1 USING (biblionumber) CROSS JOIN biblioitems USING (biblioitemnumber) LEFT JOIN biblio_metadata ON biblio_metadata.biblionumber = b1.biblionumber AND format = 'marcxml' ";
