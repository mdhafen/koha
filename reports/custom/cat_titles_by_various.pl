...
my @parameters = (
    ...
    },
    {
        ...
        select_box => 1,
        select_loop => [ map +{ value => $_->authorised_value, label => $_->lib }, Koha::AuthorisedValues->search_with_library_limits({ category => 'LOC' },{order_by=>['lib']},(C4::Context->only_my_library ? C4::Context->userenv->{branch} : '')) ],
        first_blank => 1,
        multiple => 1,
    },
    {
...
my $query = "";
my @query_params;
my @columns = ( "CONCAT_WS(' ',biblio.title,biblio.subtitle) AS title",
            "biblio.author", "biblio.seriestitle", "biblioitems.volume",
            "biblio.copyrightdate", "items.homebranch", "items.itemcallnumber",
            "items.barcode", "items.itype", "items.itemnotes",
...
if ( $do_it ) {
    ...
    if ( $input->param('item_note') ) {
        ...
        push @filters, { crit => 'items.itemnotes', op => 'LIKE', filter => '?', label => 'Copy notes', value => $note };
        push @query_params, '%'. $note .'%';
    }
    ...
    if ( $input->param('only_one') ) {
        $columns[5] = 'GROUP_CONCAT(DISTINCT items.homebranch) AS branch';
        $columns[6] = 'GROUP_CONCAT(DISTINCT items.itemcallnumber) AS itemcallnumber';
        $columns[7] = 'GROUP_CONCAT(DISTINCT items.barcode) AS barcode';
        $columns[8] = 'GROUP_CONCAT(DISTINCT items.itype) AS itype';
        $columns[9] = 'GROUP_CONCAT(DISTINCT items.itemnotes) AS itemnotes';
        $columns[10] = 'MAX(items.itemnumber) AS itemnumber';
    ...
    $query = "SELECT ". (join ',',@columns) ." FROM items CROSS JOIN biblio USING (biblionumber) CROSS JOIN biblioitems USING (biblioitemnumber) LEFT JOIN biblio_metadata ON biblio_metadata.biblionumber = biblio.biblionumber AND format = 'marcxml' ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[0]->{link} = "/cgi-bin/koha/catalogue/detail.pl?biblionumber=$values[11]";
