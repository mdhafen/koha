...
my @parameters = (
    ...
    },
    {
        ...
        op => '=',
        select_box => 1,
        select_loop => [ map +{ value => $_->authorised_value, label => $_->lib }, grep {$_->lib !~ /via SIP2$/} @{Koha::AuthorisedValues->search_with_library_limits({category=>'PAYMENT_TYPE'},{order_by=>['lib']},$branch)->as_list} ],
        first_blank => 1,
        multiple => 1,
    },
);
...
if ( $do_it ) {
    ...
    if ( $input->param('payment_types') ) {
        ...
        push @filters, { crit => 'al1.payment_type', op => 'IN', filter => '('. (join ',', map { $dbh->quote($_) } @pay_types ) .')', label => 'Payment Types', value => join ',',@pay_types };
    }
