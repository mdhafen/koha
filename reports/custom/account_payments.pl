...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[3]->{value} = sprintf( "%.2f", $values[3] );
        $results[8]->{value} = sprintf( "%.2f", $values[8] );
        ...
        $total1 += $results[ 3 ]->{value};
        $total2 += $results[ 8 ]->{value};
