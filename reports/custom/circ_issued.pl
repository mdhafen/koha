...
my $query = "";
my @query_params;
my @columns = ( "borrowers.sort2", "issues.date_due", "CONCAT( borrowers.surname, ', ', borrowers.firstname ) AS borrower", "branches.branchname", "CONCAT_WS(' ',biblio.title,biblio.subtitle) AS title", "items.itemcallnumber", "items.barcode", "COALESCE(items.replacementprice,0)", "CONCAT_WS('\n',items.itemnotes,items.itemnotes_nonpublic) AS itemnotes", "borrowers.borrowernumber" );
my $where = "";
