...
$template->param(
    report_file => 'cat_newcopies',
    report_title => 'New copies',
    report_desc => '',
    do_it => $do_it,
...
my @parameters = (
    ...
    },
    {
        ...
        select_loop => [
            ...
            { value => 'dateaccessioned', label => 'Date Added' },
        ],
    },
);

my @column_titles = ( "Title", "Author", "Series", "Volumn", "Call Number", "Barcode", "Added" );

my $query = "";
my @query_params;
my @columns = ( "CONCAT_WS(' ', biblio.title, biblio.subtitle ) AS title", "author", "seriestitle", "volume", "itemcallnumber", "barcode", "dateaccessioned", "biblionumber" );
my $where = "";
my $order = "title";
my $group = "";
...
if ( $do_it ) {
    ...
    for ( scalar $input->param("order") ) {
        ...
        elsif ( $_ eq 'dateaccessioned' ) { $order = 'dateaccessioned,title' }
    }
    ...
    $query = "SELECT ". (join ',',@columns) ." FROM items CROSS JOIN biblio USING (biblionumber) CROSS JOIN biblioitems USING (biblionumber) ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[0]->{link} = "/cgi-bin/koha/catalogue/detail.pl?biblionumber=$values[3]";
        $results[6]->{'is_date'} = 1;
