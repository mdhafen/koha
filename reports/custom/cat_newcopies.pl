...
my $reportname = "cat_newcopies";
my $reporttitle = "New Copies";
my @columns = ( "CONCAT_WS(' ', biblio.title, biblio.remainderoftitle ) AS title", "itemcallnumber", "barcode" );
...
push @queryfilter, { crit => 'dateaccessioned', op => ">", filter => $dbh->quote( C4::Dates->new( $filters[0] )->output('iso') ), title => "Date Entered", value => $filters[0] } if ( $filters[0] );
