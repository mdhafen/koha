...
$template->param(
    report_file => 'cat_items_noitype',
    report_title => 'Items with no item type',
    report_desc => '',
    do_it => $do_it,
...
my $query = "";
my @query_params;
my @columns = ( "items.homebranch", "items.barcode", "items.itemcallnumber", "CONCAT_WS(' ',biblio.title,biblio.subtitle) AS title", "items.itemnumber", "items.biblionumber" );
my $where = "( itype IS NULL OR itype = '' )";
my $order = "homebranch,title";
...
if ( $do_it ) {
    ...
    $query = "SELECT ". (join ',',@columns) ." FROM items CROSS JOIN biblio USING (biblionumber) CROSS JOIN biblioitems USING (biblioitemnumber) WHERE $where ";
...
sub calculate {
    ...
    while ( my ( @values ) = $sth->fetchrow ) {
        ...
        $results[1]->{link} = "/cgi-bin/koha/cataloguing/additem.pl?op=edititem&biblionumber=$values[5]&itemnumber=$values[4]";
